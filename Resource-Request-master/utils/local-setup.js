...
async function main() {
  try {
    ...
    const crossDomainE2ETestTestEnvironment = createCrossDomainE2ETestEnvironment(domainServiceName.declared, domainServiceEnvironment, umbrellaApprouterEnvironment);
    ...
    await writePropertiesToFile([domainServiceName.deployed, umbrellaApprouterName.deployed], `${paths.umbrella.crossDomainE2ETests}.example`, paths.umbrella.crossDomainE2ETests, crossDomainE2ETestTestEnvironment);

    if (existsSync(paths.domain.emptyPostmanEnvironment)) {
      const postmanEnvironment = await createPostmanEnvironment(domainApprouterEnvironment ? domainApprouterName.declared : umbrellaApprouterName.declared, domainServiceEnvironment, domainApprouterEnvironment || umbrellaApprouterEnvironment);
      await writePropertiesToFile([domainServiceEnvironment ? domainServiceName.deployed, domainApprouterEnvironment ? domainApprouterName.deployed : umbrellaApprouterName.deployed], null, `${paths.domain.utils}/${(postmanEnvironment || { name: '*' }).name}.postman_environment.json`, postmanEnvironment);
    } else {
