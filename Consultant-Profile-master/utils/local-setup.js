...
const paths = {
  ...
  domain: {
    ...
    srv: __dirname + '/../srv',
    integrationSrv: __dirname + '/../integration-srv',
    ...
    dbIntegrationTests: __dirname + '/../tests/db-integration/default-env.json'
  }
}
...
async function main() {
    try {
        ...
        const umbrellaConsultantProfileServiceName = await getAppNameForPath(cloudFoundryEnvironments, paths.umbrella.mtaYaml, 'Consultant-Profile\\/srv');
        const umbrellaConsultantProfileIntegrationServiceName = await getAppNameForPath(cloudFoundryEnvironments, paths.umbrella.mtaYaml, 'Consultant-Profile\\/integration-srv')
        const domainServiceName = await getAppNameForPath(cloudFoundryEnvironments, paths.domain.mtaYaml, 'srv');
        const domainIntegrationServiceName = await getAppNameForPath(cloudFoundryEnvironments, paths.domain.mtaYaml, 'integration-srv');
        ...
        const umbrellaConsultantProfileServiceEnvironment = cloudFoundryEnvironments.get(umbrellaConsultantProfileServiceName.deployed);
        const umbrellaConsultantProfileIntegrationServiceEnvironment = cloudFoundryEnvironments.get(umbrellaConsultantProfileIntegrationServiceName.deployed);
        const domainServiceEnvironment = cloudFoundryEnvironments.get(domainServiceName.deployed);
        const domainIntegrationServiceEnvironment = cloudFoundryEnvironments.get(domainIntegrationServiceName.deployed);

        const apiIntegrationTestEnvironment = createApiIntegrationTestEnvironment(domainServiceName.declared, domainIntegrationServiceName.declared, domainServiceEnvironment);
        ...
        const dbIntegrationTestEnvironment = createDbIntegrationTestEnironment(domainServiceName.declared, domainServiceEnvironment);
        const crossDomainE2ETestTestEnvironment = createCrossDomainE2ETestEnvironment(domainServiceName.declared, umbrellaConsultantProfileServiceEnvironment, umbrellaApprouterEnvironment);
        ...
        await writePropertiesToFile([domainIntegrationServiceName.deployed], null, `${paths.domain.integrationSrv}/default-env.json`, domainIntegrationServiceEnvironment);
        ...
        await writePropertiesToFile([domainServiceName.deployed], `${paths.domain.dbIntegrationTests}.example`, paths.domain.dbIntegrationTests, dbIntegrationTestEnvironment);
        ...
        await writePropertiesToFile([umbrellaConsultantProfileServiceName.deployed, umbrellaApprouterName.deployed], `${paths.umbrella.crossDomainE2ETests}.example`, paths.umbrella.crossDomainE2ETests, crossDomainE2ETestTestEnvironment);
...
function createApiIntegrationTestEnvironment(srvAppName, integrationSrvAppName, serviceEnvironment) {
    ...
    return {
        ...
        API_INTEGRATION_TESTS_APP_NAME: srvAppName,
        API_INTEGRATION_TESTS_INTEGRATION_APP_NAME: integrationSrvAppName
    };
}
...
function createDbIntegrationTestEnironment(srvAppName, serviceEnvironment){
  if (!serviceEnvironment) {
    return null;
  }
  return {
    DB_INTEGRATION_TESTS_LANDSCAPE: 'sap',
    DB_INTEGRATION_TESTS_SPACE_GUID: serviceEnvironment.VCAP_APPLICATION.space_id,
    DB_INTEGRATION_TESTS_APP_NAME: srvAppName
  };
}

async function createPostmanEnvironment(approuterName, serviceEnvironment, approuterEnvironment) {
    if (!serviceEnvironment || !approuterEnvironment) {
        return null;
    }

    const [, managedHanaUrl, managedHanaBinding] = /(https:\/\/[^/]*)\/managed\/managed_instances\/(.*)/.exec(serviceEnvironment.VCAP_SERVICES['managed-hana'][0].credentials.get_all_managed_instances_url);
    ...
    const newValues = [
        ...
        {
          key: 'tenantId',
          value: serviceEnvironment.VCAP_SERVICES.xsuaa[0].credentials.tenantid,
          enabled: true
        },
        {
          key: 'xsuaaUrl',
          value: approuterEnvironment.VCAP_SERVICES.xsuaa[0].credentials.url,
          enabled: true
        },
        {
          key: 'xsuaaClientId',
          value: serviceEnvironment.VCAP_SERVICES.xsuaa[0].credentials.clientid,
          enabled: true
        },
        {
          key: 'xsuaaClientSecret',
          value: serviceEnvironment.VCAP_SERVICES.xsuaa[0].credentials.clientsecret,
          enabled: true
        },
        {
          key: 'saasRegistryUrl',
          value: approuterEnvironment.VCAP_SERVICES['saas-registry'][0].credentials.saas_registry_url,
          enabled: true
        },
        {
          key: 'saasRegistryClientId',
          value: approuterEnvironment.VCAP_SERVICES['saas-registry'][0].credentials.clientid,
          enabled: true
        },
        {
          key: 'saasRegistryClientSecret',
          value: approuterEnvironment.VCAP_SERVICES['saas-registry'][0].credentials.clientsecret,
          enabled: true
        },
        {
          key: 'managedHanaUrl',
        ...
        {
          key: 'managedHanaUser',
          value: serviceEnvironment.VCAP_SERVICES['managed-hana'][0].credentials.user,
          enabled: true
        },
        {
          key: 'managedHanaPassword',
          value: serviceEnvironment.VCAP_SERVICES['managed-hana'][0].credentials.password,
          enabled: true
        }
    ];
