...
@suite('MyResourcesService/Profiles')
export class MyResourcesProfileDataTest extends MyResourcesServiceRepository {
    public constructor() { super('Profiles'); }
    ...
    async 'Get a list of all consultant\'s profile.'() {
        ...
        const expectedProfiles: ProfileData[] = [];
        expectedProfiles.push(this.prepareExpected({
            workforcePerson: workforcePersonWithDescription3, email: email5, photo: photo4, phone: phone5, profileDetail: profileDetail5, jobDetail: jobDetail8, managerWorkAssignment: workAssignment4, resourceOrganization: resourceOrganization1, costCenter: costCenter1, costCenterAttribute: costCenterAttribute1,
        }));
    ...
    async 'Get a list of all consultant\'s profile with manager details.'() {
        ...
        for (let i = 0; i < profiles.length; i += 1) {
            const employee = profiles[i] as ProfileData;
            if (employee.workerExternalID === 'workforcePersonExternalID3') {
                assert.isNull(employee.toManager, 'Expected manger data should be null');
            }
        }
        assert.equal(response.status, 200, 'Expected status code should be 200 (Ok).');
        assert.isDefined(profiles, 'Expected a list of consultant\'s profile.');
        expect(profiles).to.deep.include.any.members([expectedProfile]);
    }
    ...
    async 'Get one consultant\'s profile.'() {
        const response = await this.get(`(${workforcePersonWithDescription3.ID})`);
        this.responses.push(response);
    ...
    async 'Get one consultant\'s profile without authorization.'() {
        const response = await this.getWithoutAuthorization(`(${workforcePersonWithDescription3.ID})`);
        this.responses.push(response);
    ...
    @test(timeout(TEST_TIMEOUT))
    async 'Get a consultant\'s profile of a different cost center.'() {
    ...
    async 'Get one consultant\'s profile with manager details.'() {
        const response = await this.get(`(${workforcePersonWithDescription3.ID})?$expand=toManager`);
        this.responses.push(response);
        const profile = response.data;
        const expectedProfile = this.prepareExpected({
            workforcePerson: workforcePersonWithDescription3, email: email5, photo: photo4, phone: phone5, profileDetail: profileDetail5, jobDetail: jobDetail8, managerWorkAssignment: workAssignment4, resourceOrganization: resourceOrganization1, costCenter: costCenter1, costCenterAttribute: costCenterAttribute1,
    ...
    async 'Get one consultant\'s profile with manager details without authorization.'() {
        const response = await this.getWithoutAuthorization(`(${workforcePersonWithDescription3.ID})?$expand=toManager`);
        this.responses.push(response);
        assert.equal(response.status, 403, 'Expected status code to be 403 (Forbidden).');
    }
    ...
    async 'Get one consultant\'s profile with worker type.'() {
        const response = await this.get(`(${workforcePersonWithDescription3.ID})?$expand=workerType`);
        this.responses.push(response);
        const profile = response.data;
        const expectedProfile = this.prepareExpected({
            workforcePerson: workforcePersonWithDescription3, email: email5, photo: photo4, phone: phone5, profileDetail: profileDetail5, jobDetail: jobDetail8, managerWorkAssignment: workAssignment4, resourceOrganization: resourceOrganization1, costCenter: costCenter1, costCenterAttribute: costCenterAttribute1,
    ...
    async 'Get one consultant\'s profile with worker type without authorization.'() {
        const response = await this.getWithoutAuthorization(`(${workforcePersonWithDescription3.ID})?$expand=workerType`);
        this.responses.push(response);
        assert.equal(response.status, 403, 'Expected status code to be 403 (Forbidden).');
    }
    ...
    async 'Deleting the consultant\'s profile is not allowed.'() {
        const response = await this.delete(`(${workforcePersonWithDescription3.ID})`);
        this.responses.push(response);
    ...
    private prepareExpected(profile: { workforcePerson: WorkforcePerson, email: Email, photo: Photo, phone: Phone, profileDetail: ProfileDetail, jobDetail: JobDetail, managerWorkAssignment: WorkAssignment, resourceOrganization: ResourceOrganizations, costCenter: CostCenter, costCenterAttribute: CostCenterAttribute },
    ...
    private prepareProfile(profile: { workforcePerson: WorkforcePerson, email: Email, photo: Photo, phone: Phone, profileDetail: ProfileDetail, jobDetail: JobDetail, managerWorkAssignment: WorkAssignment, resourceOrganization: ResourceOrganizations, costCenter: CostCenter, costCenterAttribute: CostCenterAttribute }) {
