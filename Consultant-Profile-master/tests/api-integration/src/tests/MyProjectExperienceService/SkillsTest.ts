...
import {
    ...
    setThreeProficiencyLevel1, setFourProficiencyLevel1,
} from '../../data';
import { createSkill, deleteSkill } from '../../data/service/myProjectExperienceService';
...
@suite('MyProjectExperienceService/Skills')
export class SkillsTest extends MyProjectExperienceServiceRepository {
    ...
    async 'Get a list of all assigned skills of consultants.'() {
        ...
        const expectedConsultantSkills = this.prepareExpected([skillAssignmentWithDescription1, skillAssignmentWithDescription3]);
    ...
    @test(timeout(TEST_TIMEOUT))
    async 'Get a list of all assigned skills of consultants as a user not present in the DB.'() {
        const response = await this.getWithNoConsultantInDB();
        this.responses.push(response);
        const consultantSkills = response.data.value as Skills[];

        assert.equal(response.status, 200, 'Expected status code should be 200 (Ok).');
        assert.isEmpty(consultantSkills, 'Expected an empty list.');
    }
    ...
    async 'Get a list of all assigned skills of consultants with skill and consultant details.'() {
        ...
        const expectedConsultantSkills = this.prepareExpected(
            [skillAssignmentWithDescription1, skillAssignmentWithDescription3],
    ...
    async 'Draft flow on assigning a skill to a consultant without passing level ID.'() {
        ...
        const responseDraftCreate = await this.createDraftEmptyData(employeeHeaderWithDescription1.ID, 'skills', emptyData);
    ...
    async 'Assign a no skill to a consultant should not be allowed.'() {
        const noSkillAssignment: Skills = {
            ID: uuid(),
            employee_ID: skillAssignmentWithDescription1.employee_ID,
        };
    ...
    async 'Get the previously assigned skill to a consultant.'() {
        ...
        expectedConsultantSkill[0].modifiedBy = email1.address;
        expectedConsultantSkill[0].createdBy = email1.address;
        assert.equal(response.status, 200, 'Expected status code should be 200 (Ok).');
    ...
    async 'Get the previously assigned skill to a consultant without authorization.'() {
        const response = await this.getWithoutAuthorization(`(ID=${createSkill.ID},IsActiveEntity=true)`);
        this.responses.push(response);
        assert.equal(response.status, 403, 'Expected status code should be 403 (Forbidden).');
    }
    ...
    async 'Get the previously assigned skill to a consultant with skill and consultant details.'() {
        ...
        const expectedConsultantSkill = this.prepareExpected([createSkill as SkillAssignment], [skillWithDescription3], { workforcePerson: workforcePersonWithDescription1, email: email1 }, [setThreeProficiencyLevel1]);
        expectedConsultantSkill[0].modifiedBy = email1.address;
        expectedConsultantSkill[0].createdBy = email1.address;
        if (expectedConsultantSkill[0].employee !== undefined) {
            expectedConsultantSkill[0].employee.modifiedBy = email1.address;
        }
    ...
    @test(timeout(TEST_TIMEOUT))
    async 'Get the previously assigned skill belonging to another consultant with skill and consultant details.'() {
        const response = await this.get(`(ID=${skillAssignmentWithDescription2.ID},IsActiveEntity=true)?$expand=employee,skill`);
        this.responses.push(response);
        const consultantSkill = response.data as Skills;

        assert.equal(response.status, 404, 'Expected status code should be 404 (Not Found).');
        assert.isUndefined(consultantSkill.ID, 'Expected no assigned skill belonging to another consultant.');
    }

    @test(timeout(TEST_TIMEOUT))
    async 'Get the previously assigned skill to a consultant with skill and consultant details without authorization.'() {
        const response = await this.getWithoutAuthorization(`(ID=${createSkill.ID},IsActiveEntity=true)?$expand=employee,skill`);
        this.responses.push(response);
    ...
    async 'Unassigning a skill from a consultant.'() {
        ...
        const consultantSkills = response.data.value as Skills[];
        const expectedConsultantSkills = this.prepareExpected([skillAssignmentWithDescription1, skillAssignmentWithDescription3]);
        expectedConsultantSkills.forEach((expectedConsultantSkill, i) => {
            consultantSkills[i].modifiedAt = null!;
            const localExpectedConsultantSkill = expectedConsultantSkill;
            localExpectedConsultantSkill.modifiedBy = email1.address;
        });
    ...
    async 'Check that a proficiency level of a existing skill is changeable'() {
        this.responses.push(await this.enableDraftEdit(skillAssignmentWithDescription1.employee_ID));
        const response = await this.update(`(ID=${skillAssignmentWithDescription1.ID},IsActiveEntity=false)`, { proficiencyLevel_ID: setOneProficiencyLevel2.ID });
        this.responses.push(response);
        const responseDraftActivate = await this.activateDraft(skillAssignmentWithDescription1.employee_ID);
        this.responses.push(responseDraftActivate);
        const activatedSkill = await this.get(`(ID=${skillAssignmentWithDescription1.ID},IsActiveEntity=true)`);
        this.responses.push(activatedSkill);

        assert.equal(response.status, 200, 'Expected status code should be 200 (OK)');
        assert.equal(activatedSkill.data.proficiencyLevel_ID, setOneProficiencyLevel2.ID, 'Expected proficiency level should be setOneProficiencyLevel2');
    }

    @test(timeout(TEST_TIMEOUT))
    async 'Check that blank skill without an level is not possible'() {
        this.responses.push(await this.enableDraftEdit(skillAssignmentWithDescription1.employee_ID));
        const response = await this.update(`(ID=${skillAssignmentWithDescription1.ID},IsActiveEntity=false)`, { proficiencyLevel_ID: null });
        this.responses.push(response);
        const responseDraftActivate = await this.activateDraft(skillAssignmentWithDescription1.employee_ID);
        this.responses.push(responseDraftActivate);
        const activeSkill = await this.get(`(ID=${skillAssignmentWithDescription1.ID},IsActiveEntity=true)`);
        ...
        assert.equal(activeSkill.data.proficiencyLevel_ID, setOneProficiencyLevel2.ID, 'The proficiency level should be identical');

        this.responses.push(await this.update(`(ID=${skillAssignmentWithDescription1.ID},IsActiveEntity=true)`, { proficiencyLevel_ID: setOneProficiencyLevel1.ID }));
        this.responses.push(await this.deleteDraft(skillAssignmentWithDescription1.employee_ID));
    }

    @test(timeout(TEST_TIMEOUT))
    async 'Check if it is possible to choose a proficiency level from other proficiency set'() {
        this.responses.push(await this.enableDraftEdit(skillAssignmentWithDescription1.employee_ID));
        const getSkill = await this.get(`(ID=${skillAssignmentWithDescription1.ID},IsActiveEntity=true)?$expand=skill,employee,profile,proficiencyLevel`);
        ...
        this.responses.push(response);
        const skillAfterUpdate = await this.get(`(ID=${skillAssignmentWithDescription1.ID},IsActiveEntity=true)?$expand=skill,employee,profile,proficiencyLevel`);
    ...
    private prepareExpected(assignedConsultantSkills: SkillAssignment[], skill?: Skill[], profile?: { workforcePerson: WorkforcePerson, email: Email }, proficiencyLevel?: ProficiencyLevel[]) {
