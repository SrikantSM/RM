...
import { createExternalWorkExperience } from '../../data/service/myProjectExperienceService';
...
@suite('MyProjectExperienceService/ExternalWorkExperience')
export class ExternalWorkExperienceTest extends MyProjectExperienceServiceRepository {
    ...
    async 'Get a list of all external work experiences.'() {
        ...
        assert.isDefined(externalWorkExperiences.find(
            ...
            'Expected an external work experience 1.',
        ));
        assert.isUndefined(externalWorkExperiences.find(
            (externalWorkExperience) => (
                externalWorkExperience.ID === externalWorkExperience2.ID
                && externalWorkExperience.companyName === externalWorkExperience2.companyName
                && externalWorkExperience.projectName === externalWorkExperience2.projectName
                && externalWorkExperience.rolePlayed === externalWorkExperience2.rolePlayed
                && externalWorkExperience.comments === externalWorkExperience2.comments
                && externalWorkExperience.employee_ID === employeeHeaderWithDescription2.ID
            ),
    ...
    @test(timeout(TEST_TIMEOUT))
    async 'Get a list of all external work experiences for a user not in the DB.'() {
        const response = await this.getWithNoConsultantInDB();
        this.responses.push(response);
        const externalWorkExperience = response.data.value as ExternalWorkExperience[];

        assert.equal(response.status, 200, 'Expected status code should be 200 (Ok).');
        assert.isEmpty(externalWorkExperience, 'Expected an empty list of external work experience.');
    }
    ...
    async 'Get a list of all external work experience with skill and consultant details.'() {
        ...
        assert.isDefined(externalWorkExperiences.find(
            ...
            'Expected an external work experience 1 with external work experience with skill and consultant details.',
        ));
    }
    ...
    async 'Get one external work experience.'() {
        const response = await this.get(`(ID=${externalWorkExperience1.ID},IsActiveEntity=true)`);
        this.responses.push(response);
    ...
    @test(timeout(TEST_TIMEOUT))
    async 'Get external work experience of another employee.'() {
        const response = await this.get(`(ID=${externalWorkExperience2.ID},IsActiveEntity=true)`);
        this.responses.push(response);
        const externalWorkExperience = response.data as ExternalWorkExperience;

        assert.equal(response.status, 404, 'Expected status code should be 404 (Not Found).');
        assert.isUndefined(externalWorkExperience.ID, 'External work experience from another employee is not expected.');
    }

    @test(timeout(TEST_TIMEOUT))
    async 'Get one external work experience without authorization.'() {
        const response = await this.getWithoutAuthorization(`(ID=${externalWorkExperience1.ID},IsActiveEntity=true)`);
        this.responses.push(response);
    ...
    async 'Get one external work experience with skill and consultant details.'() {
        const response = await this.get(`(ID=${externalWorkExperience1.ID},IsActiveEntity=true)?$expand=externalWorkExperienceSkills,employee,profile`);
        this.responses.push(response);
        const externalWorkExperience = response.data as ExternalWorkExperience;
    ...
    @test(timeout(TEST_TIMEOUT))
    async 'Get external work experience with skill and consultant details of another employee.'() {
        const response = await this.get(`(ID=${externalWorkExperience2.ID},IsActiveEntity=true)?$expand=externalWorkExperienceSkills,employee,profile`);
        this.responses.push(response);
        const externalWorkExperience = response.data as ExternalWorkExperience;
    ...
    async 'Get one external work experience with skill and consultant details without authorization.'() {
        const response = await this.getWithoutAuthorization(`(ID=${externalWorkExperience1.ID},IsActiveEntity=true)?$expand=externalWorkExperienceSkills,employee,profile`);
        this.responses.push(response);
    ...
    async 'Updating comments in an external work experience.'() {
        this.responses.push(await this.enableDraftEdit(externalWorkExperience1.employee_ID));
        const response = await this.update(`(ID=${externalWorkExperience1.ID},IsActiveEntity=false)`, { comments: 'New Comments' });
        this.responses.push(response);
        const responseDraftActivate = await this.activateDraft(externalWorkExperience1.employee_ID);
        this.responses.push(responseDraftActivate);
