...
export abstract class AvailabilityUploadServiceRepository extends ServiceEndPointsRepository {
    private readonly serviceName: string = 'AvailabilityUploadService';
    ...
    async 'AvailabilityUpload service availability check.'() {
        ...
        assert.equal(response.status, 200, 'AvailabilityUpload service is not available. Expected response status should be 200(Ok).');
    }
