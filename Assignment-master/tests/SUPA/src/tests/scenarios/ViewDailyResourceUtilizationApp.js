...
const tableLocators = require("../../../../locators/tableLocators.js");
...
function executeTest(supaHelper) {
	it("should set data", async function () {
		...
		clickM = browser.actions().sendKeys("a");
	...
	// 017 Filter for 2 additional service orgs
	...
	// 025  change time period to next 8 Weeks
	it("Should change time period to next 8 Weeks", function () {
		...
		if (supaHelper) {
			browser.controlFlow().execute(function () {
				supaHelper.startSupaMeasurement("025 Change time period to next 8 weeks");
			});
		}
		enter.perform();
		if (supaHelper) {
	...
	// 045 Change time period to next 28 days
	it("Should Change time period to past and Next 2 weeks", function () {
		...
		if (supaHelper) {
			browser.controlFlow().execute(function () {
				supaHelper.startSupaMeasurement("045  Change time period to past and Next 2 weeks");
			});
		}
		enter.perform();
		if (supaHelper) {
	...
	// O70 Change View to Monthly
	it("Should Change to Monthly View", function () {
		if (supaHelper) {
			browser.controlFlow().execute(function () {
				supaHelper.startSupaMeasurement("070 Change View to Monthly");
			});
		}
