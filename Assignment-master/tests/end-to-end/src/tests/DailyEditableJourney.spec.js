...
describe("Daily Editable Journey", function () {
	...
	testHelper.it("Change View to Daily ", function () {
	...
	testHelper.it("Edit & Save Assignment", function () {
		...
		When.onTheTablePage.editAssignment(10, "/rows/0/assignments/0/utilization/1");
		Then.onTheTablePage.theAssignmentBookedCapacityIs("10", "/rows/0/assignments/0/utilization/1");
		When.onThePagePage.pressSaveButton();
		Then.onThePagePage.theErrorPopoverIsNotVisible();
	});

	testHelper.it("Should be able to change date range to " + sDate, function () {
