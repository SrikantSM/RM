...
import static com.sap.c4p.rm.skill.mdiintegration.TestConstants.DESTINATION_NOT_FOUND;
...
public class TestHelper {
  ...
  public static void assertHttpClientError(ILoggingEvent errorLog, HttpMethod httpMethod, String url,
    ...
    assertEquals(StringFormatter.format("{0} {1} returned error code {2}. Response Body: {3} Message: {4}",
        httpMethod.toString(), url, 400, errorResponse, SOMETHING_WENT_WRONG), errorLog.getFormattedMessage());
  }
  ...
  public static void assertUnableProcureToken(ILoggingEvent errorLog, int rawStatusCode) {
    assertEquals(Level.ERROR, errorLog.getLevel());
    assertEquals(StringFormatter.format("Unable to procure credentials token, and the HttpStatus: {0}", rawStatusCode),
