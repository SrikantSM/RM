...
public class MessagingInjectionValidatorTest {
  ...
  private InjectionValidator<CdsData, Skills_> cut;
  ...
  public void setUp() {
    ...
    this.cut = mock(InjectionValidator.class, Mockito.withSettings().useConstructor(this.mockMessages, Skills_.class)
  ...
  public void validateForInjectionHtmlTagsDetected() {
    ...
    doReturn(singletonList(new ValuePath<>(EVIL_SCRIPT_TAG, Skills_::externalID))).when(this.cut)
  ...
  public void validateForInjectionForbiddenFirstCharactersDetected() {
    ...
    doReturn(singletonList(new ValuePath<>(EVIL_CSV, Skills_::externalID))).when(this.cut)
  ...
  public void validateForInjectionWithoutEvilInjection() {
    doReturn(CSV_INJECTION_MESSAGE_KEY).when(this.cut).getMessageKeyForCsvInjection();
    doReturn(HTML_INJECTION_MESSAGE_KEY).when(this.cut).getMessageKeyForHtmlInjection();
    doReturn(singletonList(new ValuePath<>(HARMLESS_VALUE, Skills_::externalID))).when(this.cut)
        .extractValuesForHtmlInjection(any());
    doReturn(singletonList(new ValuePath<>(HARMLESS_VALUE, Skills_::externalID))).when(this.cut)
        .extractValuesForCsvInjection(any());
