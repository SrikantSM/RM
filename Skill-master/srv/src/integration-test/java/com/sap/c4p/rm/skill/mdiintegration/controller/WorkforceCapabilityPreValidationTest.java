...
class WorkforceCapabilityPreValidationTest {
  ...
  public WorkforceCapabilityPreValidationTest(@Value("${local.server.port}") int localPort,
  ...
  @Test
  @Order(1)
  @DisplayName("Test pre-validation if MDI Replication is allowed")
  void testPreValidationIfMDIReplicationIsAllowed() throws IOException, InterruptedException {
    MultiValueMap<String, String> queryParameters = new LinkedMultiValueMap<>();
    queryParameters.add(PAGE_SIZE, "3");
    // insert catalog RM data
    this.integrationTestHelper.insertRMCatalogData();
    ...
    try {
      ...
      HttpStatus expStatus = HttpStatus.valueOf(412);
      assertEquals(expStatus, webClientResponseException.getStatusCode());
    }
    // delete catalog RM data
    this.integrationTestHelper.deleteRMCatalogData();
  }
}
