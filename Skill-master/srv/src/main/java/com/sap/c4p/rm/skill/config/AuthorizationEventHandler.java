...
public class AuthorizationEventHandler implements EventHandler, MessageSourceAware {
  ...
  public void afterServiceAuthorization(ServiceAccessEventContext context) {
    ...
    LOGGER.debug(MARKER,
        ...
        Optional.ofNullable(context.getParameterInfo().getCorrelationId()).orElse(""));
    auditLogService.logSecurityEvent(message);
  }
  ...
  public void afterEntityAuthorization(EntityAccessEventContext context) {
    ...
    String message = "";
    if (Boolean.TRUE.equals(context.getResult()))
      message = buildLogMessage(MessageKeys.AUTHORIZATION_SUCCESS_ENTITY, LOG_LEVEL_INFO, context.getAccessEntityName(),
          context.getAccessEventName(), Optional.ofNullable(context.getParameterInfo().getCorrelationId()).orElse(""));
    else
      message = buildLogMessage(MessageKeys.AUTHORIZATION_FAILURE_ENTITY, LOG_LEVEL_ERROR,
