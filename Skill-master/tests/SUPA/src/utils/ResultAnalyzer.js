...
module.exports = class ResultAnalyzer {
  ...
  async readDataFromExcel(sFileName, sSheetName) {
    ...
    const worksheet = workbook.getWorksheet(sSheetName);
    ...
    Object.keys(data).forEach((step) => {
      const individualData = data[step];
  ...
  */
  ...
  getExcelFileName() {
    ...
    // return "2201.xlsx";
    const basePath = '/home/supa/testautomation/tests/SUPA/target';
  ...
  async identifyRegression() {
    // Get Excel File Path
    const excelFilePath = this.getExcelFileName();

    // Read Data From Excel
    const data = await this.readDataFromExcel(excelFilePath, 'MeasuredData');
...
// new ResultAnalyzer("F4704_Skill").identifyRegression();