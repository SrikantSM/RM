...
async function main() {
  try {
    ...
    const umbrellaSkillServiceName = await getAppNameForPath(cloudFoundryEnvironments, paths.umbrella.mtaYaml, 'Skill\\/srv');
    ...
    const umbrellaSkillServiceEnvironment = cloudFoundryEnvironments.get(umbrellaSkillServiceName.deployed);
    ...
    const crossDomainE2ETestTestEnvironment = createCrossDomainE2ETestEnvironment(domainServiceName.declared, umbrellaSkillServiceEnvironment, umbrellaApprouterEnvironment);

    // Write App Environments
    ...
    await writePropertiesToFile([umbrellaSkillServiceName.deployed, umbrellaApprouterName.deployed], `${paths.umbrella.crossDomainE2ETests}.example`, paths.umbrella.crossDomainE2ETests, crossDomainE2ETestTestEnvironment);

    // Write Postman Environment
    if (existsSync(paths.domain.emptyPostmanEnvironment)) {
      const postmanEnvironment = await createPostmanEnvironment(domainApprouterEnvironment ? domainApprouterName.declared : umbrellaApprouterName.declared, domainServiceEnvironment || umbrellaSkillServiceEnvironment, domainApprouterEnvironment || umbrellaApprouterEnvironment);
      await writePropertiesToFile([domainServiceEnvironment ? domainServiceName.deployed : umbrellaSkillServiceName.deployed, domainApprouterEnvironment ? domainApprouterName.deployed : umbrellaApprouterName.deployed], null, `${paths.domain.utils}/${(postmanEnvironment || { name: '*' }).name}.postman_environment.json`, postmanEnvironment);
    } else {
...
async function createPostmanEnvironment(approuterName, serviceEnvironment, approuterEnvironment) {
  ...
  const serviceManagerKey = await getServiceKey(serviceEnvironment.VCAP_SERVICES['service-manager'][0]);
  ...
  const { values } = JSON.parse(await fs.readFile(paths.domain.emptyPostmanEnvironment, 'utf8'));
